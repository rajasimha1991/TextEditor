...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		HashSet<String> visited = new HashSet<String>(); // A set tracking all the visited strings
		
		root = new WPTreeNode(word1,null);
		visited.add(word1);
		queue.add(root);
		
		//while queue is not empty & word2 is not yet found
		while(!queue.isEmpty() && !visited.contains(word2)){
			...
			for(String s: neighbors){
				if(!visited.contains(s)){
					...
					visited.add(s);
					queue.add(child);
					if(s.equals(word2)) return child.buildPathToRoot();
				}
			}
